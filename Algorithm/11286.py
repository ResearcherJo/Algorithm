...
def heappush(n):
    ...
    while current>1:#부모가 있을 때까지
        if abs(heap[current])<=abs(heap[current//2]):#current가 부모보다 작다면 부모랑 값을 바꾸자.
            if abs(heap[current//2])>abs(heap[current]) or heap[current]<=heap[current//2]:
...
def heappop():
    ...
    while child<len(heap): #자식이 있다면 반복
        ...
        if child+1<len(heap) and abs(heap[child])>=abs(heap[child+1]):#오른쪽 자식이 있고, 오른쪽 자식이 왼쪽 자식보다 작다면 오른쪽 자식을 가리키게
            if abs(heap[child])>abs(heap[child+1]) or heap[child]>=heap[child+1]:
                child += 1
            
        if abs(heap[current])>=abs(heap[child]):
            if heap[current] >= heap[child] or abs(heap[current])>abs(heap[child]):#current값이 자식 값보다 크다면 자식 값과 바꾸고, current가 자식을 바라보게 한다. child는 current의 지삭을 본다.
